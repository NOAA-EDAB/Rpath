...
object. In this vignette we use the example Rpath models from Whitehouse and 
Aydin (2020) for the eastern Bering Sea (EBS), eastern Chukchi Sea (ECS), and Gulf 
of Alaska (GOA) marine food webs. Each of these models has 50–54 functional groups, 
including one fishery, one primary producer, two detrital compartments, and there 
are no stanzas.
...
include biomass, P/B, Q/B, diet composition, and natural mortality (a.k.a., M zero).
...
Each Ecosim parameter can be subject to an initial simulation, also known as a 
...
a loop. We first need to determine the number of parameter sets we wish to generate
and create lists to store those parameter sets.
```{r , echo=TRUE}
NUM_RUNS <- 1000 # how many systems to generate
...
```
Now run the generator loop. In this example we will generate 2,000 parameter sets
...
The following loop will generate a set of Ecosim parameters for the EBS food web 
model, assign the generated base biomass (B_BaseRef) to the starting biomass in 
the `rsim.scenario` object, set the burn years to 50, run a simulation with `rsim.run`, 
and reject or retain parameter sets based on the thermodynamic boundaries 
described above.
...
were responsible for the crash. We have suppressed that output off here to save 
space but here is a sample of what that console output looks like:

  1 : fail in year  1 :  Salmon returnin\
...
To determine which of the generated systems survived burn-in, how many survived 
...
boxplot(pol_perturb_out, outline=FALSE, las=2, cex.axis=0.6)
abline(h=1, lty=2)
```
