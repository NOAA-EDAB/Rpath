...
of Alaska (GOA) marine food webs, and use the EBS model in our example. Each of 
these models has 50–54 functional groups, including one fishery, one primary 
producer, two detrital compartments, and there are no stanzas. Load the unbalanced
models, balance them, and setup the rsim scenario objects.
...
burn-in, to eliminate unstable parameter sets. This instability usually results 
from incompatible draws of parameter sets that either lead to uncontrolled
...
create lists to store those parameter sets.
```{r , echo=TRUE}
...
The 52nd ecosystem was the first to survive the 50 year burn-in.
