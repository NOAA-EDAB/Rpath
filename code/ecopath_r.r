...
ecopath <- function(modfile, dietfile, pedfile, outname = FALSE){
  ...
  model <- read.csv(modfile)  # Basic parameters, detritus fate, catch, discards in that order
  diet  <- read.csv(dietfile) # diet matrix
  ped   <- read.csv(pedfile)  # pedigree file

  diet <- diet[, -1]   # Remove first column of diet matrix (assumed to be names)
  diet[is.na(diet)] <- 0
  ped <- ped[, -1]     # Remove first column of pedigree (assumed to be names)

  # Get number of groups, living, dead, and gear.  Must be input in that order
  ngroups <- length(model$Type)
  nliving <- length(model$Type[model$Type <  2])
  ndead   <- length(model$Type[model$Type == 2])
  ngear   <- length(model$Type[model$Type == 3])

  nodetrdiet <- diet[1:nliving, ]
  model$DetInput[is.na(model$DetInput)] <- 0

  # fill in GE and QB from inputs
  GE <- ifelse(is.na(model$ProdCons), model$PB / model$QB, model$ProdCons)
  QB <- ifelse(is.na(model$QB),       model$QB <- model$PB / GE, model$QB)
  model$QB <- QB
  ...
  balanced <- list(Group = model$Group, TL = TL, Biomass = Bplus, PB = PBplus, 
                   QB = QBplus, EE = EEplus, GE = GE, Removals = RemPlus)
  ...
  morts   <- list(Group = model$Group[model$Type < 3], PB = model$PB[model$Type < 3], 
                  M0 = M0plus, F = gearF[1:(nliving + ndead), ], M2 = predM)
