...
#'
#'Performs initial mass balance using a \code{Rpath.params()} file
#'
...
rpath <- function(Rpath.params, eco.name = NA, eco.area = 1) {
  ...
  discardmat <- model[,
    (10 + ndead + 1 + ngear):(10 + ndead + (2 * ngear)),
    with = F
  ]
  ...
  if (any(model$Type == 0 & is.na(model$QB) & is.na(model$ProdCons))) {
    stop(
      "A consumer is missing both QB and ProdCons - balance failed. Use check.rpath.params() to diagnose."
    )
  }
  ...
  if (any(is.na(A))) {
    stop(
      "Model is missing parameters - can't be balanced. Use check.rpath.params() to diagnose."
    )
  }
  ...
  QBplus <- model[, QB]
  QBplus[
    is.na(QBplus) & PBplus > 0.0 & !(is.na(GE) | is.nan(GE) | is.infinite(GE))
  ] <-
    (PBplus / GE)[
      is.na(QBplus) & PBplus > 0.0 & !(is.na(GE) | is.nan(GE) | is.infinite(GE))
    ]
  ...
  gearF <- as.matrix(totcatchmat) /
    living[, Biomass][row(as.matrix(totcatchmat))]
  ...
  dimnames(dietm) <- list(
    c(gnames[1:(nliving + ndead)], "Import"),
    gnames[1:nliving]
  )
  dietm[is.na(dietm)] <- 0
...
rpath.stanzas <- function(Rpath.params) {
  ...
  if (Rpath.params$stanza$NStanzaGroups == 0) {
    return(Rpath.params)
  }
  ...
  for (isp in 1:Nsplit) {
    ...
    StGroup <- data.table(
      age = stanzafile[StGroupNum == isp & StanzaNum == 1, First]:lastmonth[isp]
    )
    ...
    for (ist in 1:nstanzas) {
      ...
      month.z <- (stanzafile[StGroupNum == isp & StanzaNum == ist, Z] +
        groupfile[StGroupNum == isp, BAB]) /
        12
      survive_L[which(StGroup$age %in% first[ist]:second[ist])] <- exp(
        -1 * month.z
      )
    }
    ...
    BioPerEgg <- StGroup[
      age %in% BaseStanza[, First]:BaseStanza[, Last],
      sum(B)
    ]
    recruits <- Rpath.params$model[Group == BaseStanza[, Group], Biomass] /
      BioPerEgg
    #Save recruits
  ...
  #Drop extra columns
  stanzafile[,
    c('bs.num', 'bs.denom', 'bs', 'qs.num', 'qs.denom', 'qs') := NULL
  ]

  #Push biomass to modfile
  for (i in 1:nrow(stanzafile)) {
    Rpath.params$model[
      Group == stanzafile[i, Group],
      Biomass := stanzafile[i, Biomass]
    ]
  }
