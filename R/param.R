...
#'
#'Creates a shell of the \code{Rpath.params} list object which contains the \emph{model}, \emph{diet},
#'\emph{multistanza}, and \emph{pedigree} parameters.
#'
...
#'
#'@return Outputs a list object of \code{Rpath.params} which are populated with values
#'  of NA or logical default values.  Values can then be filled in using
#'  R.  Use \code{\link{check.rpath.params}()} to ensure parameter files are filled out
...
create.rpath.params <- function(group, type, stgroup = NA) {
  ...
  for (i in 1:length(fleet.group)) {
    model[, V1 := c(rep(0, length(group) - length(fleet.group)), rep(NA, length(fleet.group)))]
...
#'  the logical checks fails. Checks include: verification that all types are represented (e.g. consumer, producer, detrital, and fleet);
#'  check if input parameters were entered correctly; check if Diet columns sum to 1. 
...
check.rpath.params <- function(Rpath.params) {
  ...
  if (ncol(Rpath.params$model) != 10 + n.dead + 2 * n.fleet) {
    warning(
      'Model does not have the correct number of column.  There should be 10
  ...
  if (length(Rpath.params$model[Type == 3 &
                                !is.na(Biomass), Group]) > 0) {
    warning(
      paste(
        Rpath.params$model[Type == 3 & !is.na(Biomass), Group],
  ...
  if (length(Rpath.params$model[Type == 3 &
    ...
    warning(
      paste(
        Rpath.params$model[Type == 3 & !is.na(PB), Group],
  ...
  if (length(Rpath.params$model[Type > 1 &
                                !is.na(ProdCons), Group]) > 0) {
    warning(
      paste(
  ...
  if (length(Rpath.params$model[is.na(QB) &
                                is.na(ProdCons) & Type < 1, Group]) > 0) {
    warning(
      paste(
        Rpath.params$model[is.na(QB) & is.na(ProdCons) & Type < 1, Group],
  ...
  if (length(Rpath.params$model[!is.na(QB) &
                                !is.na(ProdCons) & Type < 1, Group]) > 0) {
    both <- Rpath.params$model[!is.na(QB) &
                                 !is.na(ProdCons) & Type < 1, Group]
    if (length(Rpath.params$model[Group %in% both &
                                  !is.na(PB), Group]) > 0) {
      warning(
        paste(
  ...
  if (length(Rpath.params$model[Type == 3 &
                                !is.na(BioAcc), Group]) > 0) {
    warning(
      paste(
        Rpath.params$model[Type == 3 & !is.na(BioAcc), Group],
  ...
  if (length(Rpath.params$model[Type == 3 &
                                !is.na(Unassim), Group]) > 0) {
    warning(
      paste(
        Rpath.params$model[Type == 3 & !is.na(Unassim), Group],
  ...
  if (length(Rpath.params$model[Type != 2 &
                                !is.na(DetInput), Group]) > 0) {
    warning(paste(
      Rpath.params$model[Type != 2 & !is.na(DetInput), Group],
  ...
  if (length(which(is.na(det.matrix))) > 0) {
    ...
    warning(
      paste(
        Rpath.params$model[na.group, Group],
  ...
  fleet.matrix.2 <- Rpath.params$model[(n.groups - n.fleet + 1):n.groups, (11 + n.dead):ncol(Rpath.params$model), with = F]
  if (length(which(!is.na(fleet.matrix.2))) > 0) {
    ...
    warning(paste(
      Rpath.params$model[not.na.group + (n.groups - n.fleet), Group],
  ...
  if (dietcol != (n.living + 1)) {
    warning(
      paste(
        dietcol,
        ' is the incorrect number of columns in diet matrix.',
        'There should be',
        n.living + 1
      )
    )
  ...
  if (!Rpath.params$diet[nrow(Rpath.params$diet), 1] == 'Import' &
    ...
    warning(
      'Diet matrix is missing the import row.  Please add "Import" as the
...
#'
#'Creates an \code{Rpath.param} object from a series of .csv files.
#'
#'@family Rpath functions
#'
#'@param modfile file location of the flat file containing the \emph{model} parameters.
#'@param dietfile file location of the flat file containing the \emph{diet} parameters.
#'@param stanzagroupfile file location of the flat file containing the \emph{group} parameters
...
#'  be created.
#'@param pedfile file location of the flat file containing the \emph{pedgigree} parameters.
#'@return Outputs an \code{Rpath.param} object that can be used for Rpath and subsequently
#'  Rsim.  (NOTE: This does function does not ensure data is correct or in the
#'  correct locations.Run \code{\link{check.rpath.param}()} to ensure the appropriate columns are
#'  present).
#'@export
read.rpath.params <- function(modfile,
                              dietfile,
                              pedfile = NA,
                              stanzagroupfile = NA,
                              stanzafile = NA) {
  ...
  if (!is.na(stanzagroupfile)) {
    ...
    Rpath.params$stanzas$stindiv       <- as.data.table(read.csv(stanzafile, header = T))
  } else {
    Rpath.params$stanzas$NStanzaGroups <- 0
    Rpath.params$stanzas$stgroups      <- data.table(
      StGroupNum  = NA,
      StanzaGroup = NA,
      nstanzas    = NA,
      VBGF_Ksp    = NA,
    ...
    Rpath.params$stanzas$stindiv       <- data.table(
      StGroupNum   = NA,
      StanzaNum   = NA,
      GroupNum    = NA,
      Group       = NA,
  ...
  if (!is.na(pedfile)) {
    ...
    Rpath.params$pedigree <- data.table(
      Group = Rpath.params$model$Group,
      B     = 1,
      PB    = 1,
      QB    = 1,
      Diet  = 1
...
#'
#'Creates a series of .csv files from an \code{Rpath.params} object.
#'
#'@family Rpath functions
#'
#'@param Rpath.params R object containing the Rpath parameters. Most likely this
#'  was created using \code{\link{create.rpath.params}()} or \code{\link{read.rpath.params}()}.
...
write.rpath.params <- function(Rpath.params, eco.name, path = '') {
  ...
  write.csv(
    Rpath.params$stanzas$stgroups,
    file = file.path(path, paste(eco.name, '_stanza_groups.csv', sep = '')),
    row.names = F
  )
  
  write.csv(
    Rpath.params$stanzas$stindiv,
  ...
  if (Rpath.params$stanzas$NStanzaGroups > 0) {
    for (isp in 1:Rpath.params$stanzas$NStanzaGroups) {
      write.csv(
        Rpath.params$stanzas$StGroup[[isp]],
        file = file.path(
          path,
          paste(
            eco.name,
