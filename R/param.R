...
#'  R.  Use \code{check.rpath.params()} to ensure parameter files are filled out
...
create.rpath.params <- function(group, type, stgroup = NA) {
  ...
  for (i in 1:length(fleet.group)) {
    model[,
      V1 := c(
        rep(0, length(group) - length(fleet.group)),
...
check.rpath.params <- function(Rpath.params) {
  ...
  fleet.matrix <- Rpath.params$model[
    1:(n.groups - n.fleet),
    (11 + n.dead):ncol(Rpath.params$model),
    with = F
  ]
  ...
  fleet.matrix.2 <- Rpath.params$model[
    (n.groups - n.fleet + 1):n.groups,
    (11 + n.dead):ncol(Rpath.params$model),
    with = F
  ]
  ...
  if (any(dietDF < 0)) {
    warning(
      'Found a negative diet value. Please make sure no diet values are negative.'
    )
    w <- w + 1
  }
  ...
  if (w == 0) {
    ...
    if (w == c) {
      cat(
        'Rpath parameters functional, though some may be recalculated during balance. \n'
      )
    } else {
...
#'  correct locations.Run \code{check.rpath.params()} to ensure the appropriate columns are
#'  present).
#'@export
read.rpath.params <- function(
  modfile,
  dietfile,
  pedfile = NA,
  stanzagroupfile = NA,
  ...
  if (!is.na(stanzagroupfile)) {
    ...
    Rpath.params$stanzas$stindiv <- as.data.table(read.csv(
      stanzafile,
      header = T
    ))
  } else {
